...
//----------------------------------------------------------------------------------------------------
// AlphaEngineApp - Application class contains all necessary components AEStd.hin order to run program correctly.
//----------------------------------------------------------------------------------------------------
...
public:
    // Constructors
    AlphaEngineApp(Context* context);
    virtual ~AlphaEngineApp();
...
private:
    ...
    {
        m_CurrentWindowSize.x_ = size.x_;
        m_CurrentWindowSize.y_ = size.y_;
    }

public:
    // Getter/Setters
    inline SharedPtr<Engine> GetEngine()
    {
        return engine_;
    }
    ...
    inline ResourceCache* GetConstantResCache()
    {
        return m_pConstantResourceCache;
    }
    inline BaseGameLogic* GetGameLogic() const
    {
        return m_pGameLogic;
    }
    inline Time* GetTimer()
    {
        return m_pTime;
    }
    ...
    inline BaseSocketManager* GetSocketManager()
    {
        return m_pBaseSocketManager;
    }
    inline void SetSocketManager(BaseSocketManager* manager)
    {
        m_pBaseSocketManager = manager;
    }
    inline NetworkEventForwarder* GetEventForwarder()
    {
        return m_pNetworkEventForwarder;
    }
    inline DbConnection* GetDatabaseConnection()
    {
        return m_pDBConnection;
    }
    inline ImGuiInterface* GetImGuiInterface()
    {
        return m_pImGuiInterface;
    };

    // Game specific getters/setters
    virtual String GetWindowIcon() = 0;
    virtual String GetWindowTitle() = 0;
    inline bool GetWindowMode()
    {
        return m_bIsWindowMode;
    }

    inline String GetCurrentWorkDirectory()
    {
        return m_CurrentWorkDirectory;
    }
    inline IntVector2 GetCurrentWindowSize()
    {
    ...
    inline GameOptions& GetGameOptions()
    {
        return m_GameOptions;
    }
    inline PlatformOS GetCurrentPlatform()
    {
...
extern AlphaEngineApp* g_pApp;						// Global pointer, which is visible for whole program.

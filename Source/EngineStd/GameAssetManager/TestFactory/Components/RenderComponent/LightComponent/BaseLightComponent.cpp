...
bool BaseLightComponent::VInit(pugi::xml_node* pData)
{
	...
	if (node)
	{
		...
		{
			URHO3D_LOGDEBUG("Invalid light type in XML file");
		}
	}
...
void BaseLightComponent::VPostInit(void)
{
	...
	if (!m_pNodeLight)
	{
		URHO3D_LOGDEBUG("Failed to create Urho3D::Light component in BaseLightComponent to Node with ID " + String(GetNode()->GetID()));
	}
	else
	{
